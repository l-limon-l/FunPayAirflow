...
def get_airflow() -> None | Airflow:
    """
    Возвращает существующий экземпляр Airflow'а.
    """
    if hasattr(Airflow, "instance"):
        return getattr(Airflow, "instance")
...
class Airflow(object):
    def __new__(cls, *args, **kwargs):
        if not hasattr(cls, "instance"):
            cls.instance = super(Airflow, cls).__new__(cls)
    ...
    def __init__(self, main_config: ConfigParser,
        ...
        # Прокси
        self.proxy = {}
        self.proxy_dict = airflow_tools.load_proxy_dict()  # прокси {0: "login:password@ip:port", 1: "ip:port"...}
        if self.MAIN_CFG["Proxy"].getboolean("enable"):
            if self.MAIN_CFG["Proxy"]["ip"] and self.MAIN_CFG["Proxy"]["port"].isnumeric():
                ...
                if proxy_str not in self.proxy_dict.values():
                    ...
                    airflow_tools.cache_proxy_dict(self.proxy_dict)

                if self.MAIN_CFG["Proxy"].getboolean("check") and not airflow_tools.check_proxy(self.proxy):
                    sys.exit()
        ...
        self.profile: FunPayAPI.types.UserProfile | None = None  # FunPay профиль для всего Airflow'а (+ хэндлеров)
        ...
        self.blacklist = airflow_tools.load_blacklist()  # ЧС.
        self.old_users = airflow_tools.load_old_users(
        ...
        self.disabled_plugins = airflow_tools.load_disabled_plugins()
        self.pinned_plugins = airflow_tools.load_pinned_plugins()
    ...
    def __init_account(self) -> None:
        ...
        while True:
            try:
                ...
                greeting_text = airflow_tools.create_greeting_text(self)
                airflow_tools.set_console_title(f"FunPay Airflow - {self.account.username} ({self.account.id})")
    ...
    def __update_profile(self, infinite_polling: bool = True, attempts: int = 0, update_telegram_profile: bool = True,
        ...
        :param update_main_profile: обновить ли информацию о профиле для всего Airflow'а (+ хэндлеров)?
    ...
    def raise_lots(self) -> int:
        ...
        for subcat in sorted(list(self.profile.get_sorted_lots(2).keys()), key=lambda x: x.category.position):
            ...
            try:
                ...
                time_delta = "" if not last_time else f" Последнее поднятие: {airflow_tools.time_to_str(new_time - last_time)} назад."
                time.sleep(1)
                self.account.raise_lots(subcat.category.id)
                ...
                if e.wait_time is not None:
                    logger.warning(_("crd_raise_time_err", subcat.category.name, error_text,
                                     airflow_tools.time_to_str(e.wait_time)))
    ...
    def parse_message_entities(self, msg_text: str) -> list[str | int | float]:
        ...
        pos = 0
        entities = []
        while entity := airflow_tools.ENTITY_RE.search(msg_text, pos=pos):
    ...
    def init(self):
        """
        Инициализирует Airflow: регистрирует хэндлеры, инициализирует и запускает Telegram бота,
    ...
    def run(self):
        """
        Запускает Airflow после инициализации. Используется для первого старта.
        """
        self.run_id += 1
        self.start_time = int(time.time())
    ...
    def start(self):
        """
        Запускает Airflow после остановки. Не используется.
        """
        self.run_id += 1
        self.run_handlers(self.pre_start_handlers, (self,))
        self.run_handlers(self.post_start_handlers, (self,))
    ...
    def stop(self):
        """
        Останавливает Airflow. Не используется.
        """
        self.run_id += 1
    ...
    def toggle_plugin(self, uuid):
        ...
        airflow_tools.cache_disabled_plugins(self.disabled_plugins)
    ...
    def pin_plugin(self, uuid):
        ...
        airflow_tools.cache_pinned_plugins(self.pinned_plugins)
