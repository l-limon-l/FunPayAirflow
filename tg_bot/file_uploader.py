...
from Utils import config_loader as cfg_loader, exceptions as excs, airflow_tools, updater
...
def init_uploader(cardinal: Cardinal):
    ...
    def upload_products_file(m: types.Message):
        ...
        try:
            products_count = airflow_tools.count_products(f"storage/products/{utils.escape(m.document.file_name)}")
        except:
