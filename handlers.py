...
from Utils import airflow_tools
...
logger = logging.getLogger("FPA.handlers")
localizer = Localizer()
...
# INIT MESSAGE
def save_init_chats_handler(c: Airflow, e: InitialChatEvent):
    """
    ...
    if c.MAIN_CFG["Greetings"].getboolean("sendGreetings") and e.chat.id not in c.old_users:
        ...
        airflow_tools.cache_old_users(c.old_users)


def update_threshold_on_initial_chat(c: Airflow, e: InitialChatEvent):
    """
...
def old_log_msg_handler(c: Airflow, e: LastChatMessageChangedEvent):
    """
...
def log_msg_handler(c: Airflow, e: NewMessageEvent):
...
def update_threshold_on_last_message_change(c: Airflow, e: LastChatMessageChangedEvent | NewMessageEvent):
    """
    Обновляет пороговое значение для определения новых чатов.
    """
...
def greetings_handler(c: Airflow, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
    ...
    text = airflow_tools.format_msg_text(c.MAIN_CFG["Greetings"]["greetingsText"], obj)
    Thread(target=c.send_message, args=(chat_id, text, chat_name), daemon=True).start()


def add_old_user_handler(c: Airflow, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
...
def send_response_handler(c: Airflow, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
    ...
    response_text = airflow_tools.format_msg_text(c.AR_CFG[command]["response"], obj)
    Thread(target=c.send_message, args=(chat_id, response_text, chat_name), daemon=True).start()


def old_send_new_msg_notification_handler(c: Airflow, e: LastChatMessageChangedEvent):
...
def send_new_msg_notification_handler(c: Airflow, e: NewMessageEvent) -> None:
    """
...
def send_review_notification(c: Airflow, order: Order, chat_id: int, reply_text: str | None):
    if not c.telegram:
        return
...
def process_review_handler(c: Airflow, e: NewMessageEvent | LastChatMessageChangedEvent):
    if not c.old_mode_enabled:
        if isinstance(e, LastChatMessageChangedEvent):
            return
        obj = e.message
    ...
    def send_reply():
        ...
        if c.MAIN_CFG["ReviewReply"].getboolean(toggle) and c.MAIN_CFG["ReviewReply"].get(text):
            try:
                ...
                reply_text = airflow_tools.format_order_text(c.MAIN_CFG["ReviewReply"].get(text), order)
...
def send_command_notification_handler(c: Airflow, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
    ...
    if not c.AR_CFG[command].get("notificationText"):
        ...
        text = airflow_tools.format_msg_text(c.AR_CFG[command]["notificationText"], obj)
...
def test_auto_delivery_handler(c: Airflow, e: NewMessageEvent | LastChatMessageChangedEvent):
    """
...
def send_categories_raised_notification_handler(c: Airflow, cat: types.Category, error_text: str = "") -> None:
    """
...
def get_lot_config_by_name(c: Airflow, name: str) -> configparser.SectionProxy | None:
    """
...
def check_products_amount(config_obj: configparser.SectionProxy) -> int:
    ...
    if not file_name:
        return 1
    return airflow_tools.count_products(f"storage/products/{file_name}")


# Новый ордер (REGISTER_TO_NEW_ORDER)
def log_new_order_handler(c: Airflow, e: NewOrderEvent, *args):
    """
...
def setup_event_attributes_handler(c: Airflow, e: NewOrderEvent, *args):
...
def send_new_order_notification_handler(c: Airflow, e: NewOrderEvent, *args):
    """
...
def deliver_goods(c: Airflow, e: NewOrderEvent, *args):
    chat = c.account.get_chat_by_name(e.order.buyer_username)
    if chat:
        chat_id = chat.id
    else:
    ...
    delivery_text = airflow_tools.format_order_text(cfg_obj["response"], e.order)
    ...
    try:
        if file_name := cfg_obj.get("productsFileName"):
            ...
            products, goods_left = airflow_tools.get_products(f"storage/products/{file_name}", amount)
    ...
    if not result:
        ...
        if file_name and products:
            airflow_tools.add_products(f"storage/products/{file_name}", products, at_zero_position=True)
    else:
...
def deliver_product_handler(c: Airflow, e: NewOrderEvent, *args) -> None:
    """
...
def send_delivery_notification_handler(c: Airflow, e: NewOrderEvent):
    """
...
def update_current_lots(c: Airflow, e: NewOrderEvent):
...
def update_profile_lots(c: Airflow, e: NewOrderEvent):
...
def update_lot_state(cardinal: Airflow, lot: types.LotShortcut, task: int) -> bool:
    """
...
def update_lots_states(cardinal: Airflow, event: NewOrderEvent):
...
def update_profiles_handler(cardinal: Airflow, event: NewOrderEvent | OrdersListChangedEvent, *args):
    """Обновляет информацию о профилях и состояния лотов в отдельном потоке."""
    def f(c: Airflow, e: NewOrderEvent):
...
def send_thank_u_message_handler(cardinal: Airflow, event: OrderStatusChangedEvent):
    """
    ...
    text = airflow_tools.format_order_text(cardinal.MAIN_CFG["OrderConfirm"]["replyText"], event.order)
...
def send_order_confirmed_notification_handler(cardinal: Airflow, event: OrderStatusChangedEvent):
    """
...
def send_bot_started_notification_handler(c: Airflow, *args):
    """
