...
Utils.airflow_tools.set_console_title(f"FunPay Airflow v{VERSION}")
...
print(f"{Fore.MAGENTA}{Style.BRIGHT}By {Fore.BLUE}{Style.BRIGHT}l_limon_l{Style.RESET_ALL}")
print(f"{Fore.MAGENTA}{Style.BRIGHT}GitHub: {Fore.BLUE}{Style.BRIGHT}https://github.com/l-limon-l/FunPayAirflow{Style.RESET_ALL}")
...
if sys.platform == "linux" and os.getenv('FPC_IS_RUNNIG_AS_SERVICE', '0') == '1':
    ...
    pid = str(os.getpid())
    pidFile = open(f"/run/FunPayAirflow/{getpass.getuser()}/FunPayAirflow.pid", "w")
    pidFile.write(pid)
    pidFile.close()
...
try:
    Airflow(MAIN_CFG, AD_CFG, AR_CFG, RAW_AR_CFG, VERSION).init().run()
