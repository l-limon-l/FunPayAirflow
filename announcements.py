...
logger = getLogger("FPA.announcements")
localizer = Localizer()
...
def get_announcement(ignore_last_tag: bool = False) -> dict | None:
    ...
    try:
        response = requests.get("https://api.github.com/gists/2412f3d834c2a16265059085e18627d0", headers=headers)
...
def announcements_loop_iteration(airflow: Airflow, ignore_last_tag: bool = False):
    global LAST_TAG
    ...
    if text or photo:
        Thread(target=airflow.telegram.send_notification,
               args=(text,),
...
def announcements_loop(crd: Airflow):
    """
    ...
    while True:
        try:
            announcements_loop_iteration(airflow, ignore_last_tag=False)
        except:
            pass
        time.sleep(REQUESTS_DELAY)


def main(airflow: Airflow):
    Thread(target=announcements_loop, args=(airflow,), daemon=True).start()
